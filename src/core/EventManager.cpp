    ...
    void EventManager::setMusic(float music, IGraphic& graph) {
        ...
        for (auto & _entitie : *_entities) {
            if (_entitie.first == "musicBarValue") {
                ...
                printf("get volume event bef int : %f\n", graph.getMusicVolume());
                int musicVolume = static_cast<int>(graph.getMusicVolume() * 10);
                printf("music volume in event after *10 : %d\n", musicVolume);
                e.setText((char*)std::to_string((int)musicVolume).c_str());
                return;
            }
        }
    }
    ...
    void EventManager::setSound(float sound, IGraphic& graph) {
        ...
        for (auto & _entitie : *_entities) {
            if (_entitie.first == "soundBarValue") {
                auto &e = _entitie.second->getComponent<TextComponent>();
                int soundVolume = graph.getSoundVolume() * 10;
                e.setText((char*)std::to_string(soundVolume).c_str());
                return;
            }
        }
    }
            ...
            if (myEvents[index]["name"] == entity.first) {
                for (int indexTwo = 0; indexTwo < myEvents.size(); indexTwo++) {
                    ...
                    if (Events[indexTwo]["name"] == "music") {
                        setMusic(Events[indexTwo]["value"].asFloat(), libGraphic);
                    }
                    if (Events[indexTwo]["name"] == "sound") {
                        setSound(Events[indexTwo]["value"].asFloat(), libGraphic);
                    }
