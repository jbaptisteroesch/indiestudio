    ...
    void IA::moveToTarget(std::array<std::string, 13> map, IGraphic &libgraphic, Component *compo, float time) {
        ...
        if (isSafe() || moveIfBombOnLine() || moveToTargetSpecialCase()) {
            ...
            if (!checkBarrier(round(_posPlayer.first) + 1, round(_posPlayer.second), RIGHTIA, true)) {
                //std::cout << "In function moveToTarget MOVE RIGHT xDist > yDist && xDist > 0" << std::endl;
                ...
                //std::cout << "In function moveToTarget MOVE DOWN xDist > yDist && xDist > 0" << std::endl;
                _mvtDirection = DOWNIA;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA) && yDist < 0) {
                //std::cout << "In function moveToTarget MOVE UP xDist > yDist && xDist > 0" << std::endl;
                _mvtDirection = UPIA;
            } else if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second)][round(_posPlayer.first) + 1] == 'B') {
                _plantBomb = true;
            } else if (!checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA)) {
                //std::cout << "In function moveToTarget MOVE LEFT xDist > yDist && xDist > 0" << std::endl;
                _mvtDirection = LEFTIA;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA)) {
                //std::cout << "In function moveToTarget MOVE UP 2 xDist > yDist && xDist > 0" << std::endl;
                _mvtDirection = UPIA;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) + 1, DOWNIA)) {
                //std::cout << "In function moveToTarget MOVE DOWN 2 xDist > yDist && xDist > 0" << std::endl;
                _mvtDirection = DOWNIA;
            }
        } else if (xDist <= yDist && yDist > 0) {
            //std::cout << "DOWNIA" << std::endl;
            if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) + 1, DOWNIA, true)) {
                //std::cout << "In function moveToTarget MOVE DOWN xDist <= yDist && yDist > 0" << std::endl;
                _mvtDirection = DOWNIA;
                ...
                //std::cout << "In function moveToTarget MOVE rRIGHT xDist <= yDist && yDist > 0" << std::endl;
                _mvtDirection = RIGHTIA;
            } else if (!checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA) && xDist < 0) {
                //std::cout << "In function moveToTarget MOVE LEFT xDist <= yDist && yDist > 0" << std::endl;
                _mvtDirection = LEFTIA;
            } else if (!_travelWall && !_safeMode && map[round(_posPlayer.second) + 1][round(_posPlayer.first)] == 'B') {
                _plantBomb = true;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA)) {
                //std::cout << "In function moveToTarget MOVE UP xDist <= yDist && yDist > 0" << std::endl;
                _mvtDirection = UPIA;
            } else if (!checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA)) {
                //std::cout << "In function moveToTarget MOVE LEFT 2 xDist <= yDist && yDist > 0" << std::endl;
                _mvtDirection = LEFTIA;
            } else if (!checkBarrier(round(_posPlayer.first) + 1, round(_posPlayer.second), RIGHTIA)) {
                //std::cout << "In function moveToTarget MOVE RIGHT 2 xDist <= yDist && yDist > 0" << std::endl;
                _mvtDirection = RIGHTIA;
            }
        } else if (xDist > yDist && yDist <= 0) {
            //std::cout << "UPIA" << std::endl;
            if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA, true)) {
                ...
                //std::cout << "In function moveToTarget MOVE RIGHT xDist > yDist && yDist <= 0" << std::endl;
                _mvtDirection = RIGHTIA;
            } else if (!_travelWall && !_safeMode && map[round(_posPlayer.second) - 1][round(_posPlayer.first)] == 'B') {
                _plantBomb = true;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) + 1, DOWNIA)) {
    ...
    bool IA::moveIfBombOnLine() {
        ...
        if (!(pairPosBombEnemy = getPosBombEnemyOnLine()).empty()) {
            for (auto & itPosBomb : pairPosBombEnemy) {
                ...
                if (yDist == 0) {
                    /*
                    if ((xDist < 0 && _myMap[itPosBomb.second][itPosBomb.first + 1] == '#' ||
                         _myMap[round(_posPlayer.second)][itPosBomb.first + 1] == 'B') ||
                        (xDist > 0 && _myMap[itPosBomb.second][itPosBomb.first - 1] == '#' ||
                         _myMap[itPosBomb.second][itPosBomb.first - 1] == 'B')) {
                        std::cout << "In function moveIfBombOnLine Nothing Happened yDist == 0" << std::endl;
                        return false;
                    } */if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA)) {
                        //std::cout << "In function moveIfBombOnLine MOVE UP yDist == 0" << std::endl;
                        _mvtDirection = UPIA;
                        return true;
                    } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) + 1, DOWNIA)) {
                        //std::cout << "In function moveIfBombOnLine MOVE DOWN yDist == 0" << std::endl;
                        _mvtDirection = DOWNIA;
                        return true;
                    } else if (xDist > 0 && !checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second))) {
                        //std::cout << "In function moveIfBombOnLine MOVE LEFT yDist == 0" << std::endl;
                        _mvtDirection = LEFTIA;
                        return true;
                    } else if (xDist < 0 && !checkBarrier(round(_posPlayer.first) + 1, round(_posPlayer.second))) {
                        //std::cout << "In function moveIfBombOnLine MOVE RIGHT yDist == 0" << std::endl;
                        _mvtDirection = RIGHTIA;
                        return true;
                    }
                } else if (xDist == 0) {
                    /*if ((yDist < 0 && _myMap[itPosBomb.second + 1][itPosBomb.first] == '#' ||
                         _myMap[itPosBomb.second + 1][itPosBomb.first] == 'B') ||
                        (yDist > 0 && _myMap[itPosBomb.second - 1][itPosBomb.first] == '#' ||
                         _myMap[itPosBomb.second - 1][itPosBomb.first] == 'B')) {
                        std::cout << "In function moveIfBombOnLine Nothing Happened xDist == 0" << std::endl;
                        return false;
                    } */if (!checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA)) {
                        //std::cout << "In function moveIfBombOnLine MOVE LEFT xDist == 0" << std::endl;
                        _mvtDirection = LEFTIA;
                        return true;
                    } else if (!checkBarrier(round(_posPlayer.first) + 1, round(_posPlayer.second), RIGHTIA)) {
                        //std::cout << "In function moveIfBombOnLine MOVE RIGHT xDist == 0" << std::endl;
                        _mvtDirection = RIGHTIA;
                        return true;
                    } else if (yDist > 0 && !checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1)) {
                        //std::cout << "In function moveIfBombOnLine MOVE UP xDist == 0" << std::endl;
                        _mvtDirection = UPIA;
                        return true;
                    } else if (yDist < 0 && !checkBarrier(round(_posPlayer.first), round(_posPlayer.second) + 1)) {
                        //std::cout << "In function moveIfBombOnLine MOVE DOWN xDist == 0" << std::endl;
                        _mvtDirection = DOWNIA;
                        return true;
                    }
                }
    ...
    bool IA::moveToTargetSpecialCase() {
        ...
        if (yDist == -1) {
            if (!checkBarrier(round(_posPlayer.first) + 1, round(_posPlayer.second), RIGHTIA, true) && xDist > 0) {
                //std::cout << "In function moveToTargetSpecialCase MOVE RIGHT yDist == -1" << std::endl;
                _mvtDirection = RIGHTIA;
                return true;
            } else if (!checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA, true) && xDist < 0) {
                //std::cout << "In function moveToTargetSpecialCase MOVE LEFT yDist == -1" << std::endl;
                _mvtDirection = LEFTIA;
                return true;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA)) {
                //std::cout << "In function moveToTargetSpecialCase MOVE UP yDist == -1" << std::endl;
                _mvtDirection = UPIA;
                return true;
            } else if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second) - 1][round(_posPlayer.first)] == 'B') {
                _plantBomb = true;
            ...
            if (!checkBarrier(round(_posPlayer.first) + 1, round(_posPlayer.second), RIGHTIA, true) && xDist > 0) {
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE DOWN yDist == 1" << std::endl;
                _mvtDirection = DOWNIA;
                return true;
            } else if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second) + 1][round(_posPlayer.first)] == 'B') {
                _plantBomb = true;
            ...
            if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) + 1, DOWNIA, true) && yDist > 0) {
                //std::cout << "In function moveToTargetSpecialCase MOVE DOWN XDist == -1" << std::endl;
                _mvtDirection = DOWNIA;
                return true;
            } else if (!checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA, true) && yDist < 0) {
                //std::cout << "In function moveToTargetSpecialCase MOVE UP XDist == -1" << std::endl;
                _mvtDirection = UPIA;
                return true;
            } else if (!checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA)) {
                //std::cout << "In function moveToTargetSpecialCase MOVE LEFT XDist == -1" << std::endl;
                _mvtDirection = LEFTIA;
                return true;
            } else if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second)][round(_posPlayer.first) - 1] == 'B') {
                //std::cout << "In function moveToTargetSpecialCase MOVE RIGHT XDist == -1" << std::endl;
                _plantBomb = true;
                return true;
            }
        } else if (xDist == 1) {
            ...
            if (xDist < 0 && !checkBarrier(round(_posPlayer.first) - 1, round(_posPlayer.second), LEFTIA)) {
                //std::cout << "In function moveToTargetSpecialCase MOVE LEFT yDist == 0" << std::endl;
                _mvtDirection = LEFTIA;
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE RIGHT yDist == 0" << std::endl;
                _mvtDirection = RIGHTIA;
                if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second)][round(_posPlayer.first) + 2] == 'B')
                    _plantBomb = true;
                return true;
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE DOWN yDist == 0" << std::endl;
                _mvtDirection = DOWNIA;
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE UP yDist == 0" << std::endl;
                _mvtDirection = UPIA;
                if (!_travelWall && !_safeMode && ((xDist < 0 && _myMap[round(_posPlayer.second)][round(_posPlayer.first) - 1] == 'B') || (xDist > 0 && _myMap[round(_posPlayer.second)][round(_posPlayer.first) + 1] == 'B')))
                    _plantBomb = true;
                return true;
            ...
            if (yDist < 0 && !checkBarrier(round(_posPlayer.first), round(_posPlayer.second) - 1, UPIA)) {
                //std::cout << "In function moveToTargetSpecialCase MOVE UP xDist == 0" << std::endl;
                _mvtDirection = UPIA;
                if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second) - 2][round(_posPlayer.first)] == 'B') {
                    _plantBomb = true;
                }
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE DOWN xDist == 0" << std::endl;
                _mvtDirection = DOWNIA;
                if (!_travelWall && !_safeMode && _myMap[round(_posPlayer.second) + 2][round(_posPlayer.first)] == 'B')
                    _plantBomb = true;
                return true;
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE RIGHT xDist == 0" << std::endl;
                _mvtDirection = RIGHTIA;
                ...
                //std::cout << "In function moveToTargetSpecialCase MOVE LEFT xDist == 0" << std::endl;
                _mvtDirection = LEFTIA;
                if (!_travelWall && !_safeMode && ((xDist < 0 && _myMap[round(_posPlayer.second) - 1][round(_posPlayer.first)] == 'B') || (xDist > 0 && _myMap[round(_posPlayer.second) + 1][round(_posPlayer.first)] == 'B')))
                    _plantBomb = true;
                return true;
